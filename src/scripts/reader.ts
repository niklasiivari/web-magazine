...
export default function initReader(): void {
  ...
  const savedLanguage = localStorage.getItem(`${site.reader.storageKey}-language`);
  ...
  let viewMode: ViewMode =
    localStorage.getItem(`${site.reader.storageKey}-view`) === "pages" ? "pages" : "articles";
  ...
  const buildArticleView = () => {
    ...
    for (const source of sources) {
      ...
      if (source.classList.contains("reader-cover-page")) {
        ...
        spread.dataset.readerCounter = title || t("credits");
        spread.dataset.readerLabel = title || t("credits");
  ...
  for (const option of viewOptions) {
    option.addEventListener("click", () => {
      ...
      localStorage.setItem(`${site.reader.storageKey}-view`, viewMode);
      mountView();
  ...
  for (const option of languageOptions) {
    option.addEventListener("click", () => {
      ...
      localStorage.setItem(`${site.reader.storageKey}-language`, uiLanguage);
      applyReaderLanguage();
      mountView();
