...
func main() {
	...
	resolveInDist := func(path string) (string, bool) {
		abs, err := filepath.Abs(path)
		if err != nil || !strings.HasPrefix(abs, absDist+string(os.PathSeparator)) {
			return "", false
		}
		return abs, true
	}
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if !strings.Contains(filepath.Base(r.URL.Path), ".") {
			for _, candidate := range []string{r.URL.Path + ".html", strings.TrimSuffix(r.URL.Path, "/") + "/index.html"} {
				full, ok := resolveInDist(filepath.Join(distDir, filepath.Clean(candidate)))
				if !ok {
					continue
				}
